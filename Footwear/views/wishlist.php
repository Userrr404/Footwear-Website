    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between mb-10">
        ...
        <?php if ($result->num_rows > 0): ?>
          <p class="text-gray-600 text-sm md:text-base mt-3 sm:mt-0">
            You have <span class="font-semibold"><?= $result->num_rows ?></span> items in your wishlist
          </p>
        <?php endif; ?>
      </div>
      ...
      <?php if ($result->num_rows > 0): ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          <?php while ($row = $result->fetch_assoc()): ?>
            <!-- Product Card -->
            <div class="bg-white rounded-2xl shadow-md hover:shadow-lg transition transform hover:-translate-y-1 overflow-hidden flex flex-col">
              ...
              <div class="flex-1 p-5 flex flex-col justify-between gap-4">
                ...
                <div class="flex items-center justify-between mt-auto gap-3">
                  ...
                  <form method="POST" action="../php/remove_from_wishlist.php" class="flex-1 m-0">
                    <input type="hidden" name="product_id" value="<?= $row['product_id'] ?>">
                    <button type="submit" 
                            class="w-full bg-gray-200 text-gray-700 text-sm font-medium px-8 py-2 rounded-lg hover:bg-red-600 hover:text-white transition">
                      Remove
                    </button>
                  </form>
                </div>
